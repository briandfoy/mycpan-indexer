# This Makefile is for the MyCPAN::Indexer extension to perl.
#
...
#     ABSTRACT => q[Index a Perl distribution]
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Indexer.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/MyCPAN
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/MyCPAN
...
TO_INST_PM = lib/Indexer.pm

PM_TO_BLIB = lib/Indexer.pm \
	$(INST_LIBDIR)/Indexer.pm
...
DISTNAME = MyCPAN-Indexer
DISTVNAME = MyCPAN-Indexer-0.10_02
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                MyCPAN-Indexer' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             0.10_02' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            Index a Perl distribution' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '    Distribution::Guess::BuildSystem: 0' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="0,10_02,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Index a Perl distribution</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>brian d foy &lt;bdfoy@cpan.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Archive-Extract" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <DEPENDENCY NAME="Distribution-Guess-BuildSystem" VERSION="0,0,0,0" />' >> $(DISTNAME).ppd
